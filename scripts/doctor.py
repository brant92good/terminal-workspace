...
def workspace_checks(root=ROOT):
    ...
    checks.append(check('herdr', isinstance(herdr, str) and Path(herdr).is_file(), 'The Herdr remote-terminal app is installed.',
                        'Install Herdr from https://herdr.dev, or pass install.ps1 -HerdrPath C:\\path\\herdr.exe.'))
    checks.append(check('workspace_installed', bool(machine.get('ssh_host')) and (root / 'build/TerminalWorkspace.exe').is_file(),
                        'This checkout has its remote target and two-tab launcher.',
                        'Run .\\install.ps1 -SshHost YOUR_SSH_NAME.'))
    return checks

"""Measure actual Ctrl+Alt+P keypress-to-content-focus latency in temporary tabs."""
import argparse
import json
...
def main():
    ...
    try:
        ...
        subprocess.Popen(["wt.exe", "-w", "new", "new-tab", "-p", PORTS, ";", "new-tab", "-p", HERDR])
        ...
        command = """param([string]$Root,[string]$Target,[string]$Source,[int]$Count)
...
[BenchmarkSwitch]::Run($Target,$Source,$Count)
"""
        import tempfile
        with tempfile.TemporaryDirectory(prefix="terminal-benchmark-") as folder:
            ...
            result = subprocess.run(["powershell.exe", "-NoProfile", "-ExecutionPolicy", "Bypass", "-File", str(script),
                ...
                check=True, capture_output=True, encoding="utf-8", creationflags=subprocess.CREATE_NO_WINDOW, timeout=90)
        samples = json.loads(result.stdout)
        report = {"samples_ms": [round(x, 1) for x in samples], "median_ms": round(statistics.median(samples), 1),

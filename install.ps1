param(
    ...
    [switch]$WorkspaceFiles, [switch]$NoWorkspaceFiles,
...
if ($WorkspaceFiles -and $NoWorkspaceFiles) { throw 'Choose WorkspaceFiles or NoWorkspaceFiles.' }
...
foreach ($workspacePair in @(@('--local-herdr',$LocalHerdr),@('--no-local-herdr',$NoLocalHerdr),@('--workspace-files',$WorkspaceFiles),@('--no-workspace-files',$NoWorkspaceFiles),@('--session-picker',$SessionPicker),@('--no-session-picker',$NoSessionPicker),@('--integration-only',$IntegrationOnly),@('--apply-shared-settings',$ApplySharedSettings))) {
